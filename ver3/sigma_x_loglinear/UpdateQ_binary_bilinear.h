             ...
             mat Sigma_p, double alpha_scale_p, int n_chunk=5)
{
    ...
    for (uword i = 0; i < ind_chunks.n_elem-1; i++){
        ...
        mat DQP_c_long = bilinearInterpolator_mat(X_QP.cols(1,2), DQP_c, X_prime.cols(1,2)); // interpolating in uniform scale based on 2 columns
        mat DQP_p_long = bilinearInterpolator_mat(X_QP.cols(1,2), DQP_p, X_prime.cols(1,2)); // interpolating in uniform scale based on 2 columns
