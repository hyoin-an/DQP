              ...
              vec mu_1, mat Sigma_1_inv, double log_Sigma_1_det, int n_chunk=5)
{
    ...
    for (uword i = 0; i < ind_chunks.n_elem-1; i++){
        ...
        vec eta_p = eta_c;
        eta_p(ind) = eta_p_ind;
        vec eta_p_long = interpolator(X_QP.col(1), eta_p, X_prime.col(1)); // **NEW**
        vec eta_c_long = interpolator(X_QP.col(1), eta_c, X_prime.col(1)); // **NEW**
        ...
        // Log Likelihood
        vec mu_x = X_QP * beta; 
        vec mu_x_long = X * beta;  
        vec sigma_x_p = exp(eta_p);
        vec sigma_x_c = exp(eta_c);
        vec sigma_x_p_long = exp(eta_p_long);
        vec sigma_x_c_long = exp(eta_c_long);
        ...
        mat DQP_u_p_long = interpolator_mat(X_QP.col(1), DQP_u_p, X_prime.col(1)); // interpolating based on the lowest order of polynomials
        mat DQP_u_c_long = interpolator_mat(X_QP.col(1), DQP_u_c, X_prime.col(1)); // interpolating based on the lowest order of polynomials
