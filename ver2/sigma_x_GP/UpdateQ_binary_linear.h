...
List UpdateQ(mat DQP_c_y, mat DQP_Z_c, vec beta, vec sigma_x, vec sigma_x_long, List quantile_levels, vec all_quantiles, double alpha_scale,
             vec y, mat X, mat X_prime, vec group_ind, mat X_QP, vec mu, mat Sigma, mat Sigma_inv, double log_Sigma_det, 
             mat Sigma_p, double alpha_scale_p, int n_chunk=5)
{
    ...
    for (uword i = 0; i < ind_chunks.n_elem-1; i++){
        ...
        mat DQP_c_long = interpolator_mat(X_QP.col(1), DQP_c, X_prime.col(1)); // interpolating based on the lowest order of polynomials
        mat DQP_p_long = interpolator_mat(X_QP.col(1), DQP_p, X_prime.col(1)); // interpolating based on the lowest order of polynomials
