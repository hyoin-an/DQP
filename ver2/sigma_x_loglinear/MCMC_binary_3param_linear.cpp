...
#include "UpdateQ_binary_linear.h"
#include "UpdateBeta_binary_linear.h"
#include "UpdateGamma_binary_linear.h"
...
// [[Rcpp::export]]
List MCMC_param3(int niter, vec y, mat x, mat x_QP, List quantile_levels, 
                 vec mu, mat Sigma, vec mu_0, mat Sigma_0, vec mu_1, mat Sigma_1, double alpha_scale, 
                 mat Sigma_p, double alpha_scale_p, mat Sigma_beta, mat Sigma_gamma, 
                 int n_chunk_Q, int n_chunk_beta, int n_chunk_gamma,
    ...
    // x = n x p, original covariate matrix (replicates O, all locations O)
    ...
    // // obtain y_QP and x_QP with all the replicates
    // mat Z = join_horiz(y, x);
    // mat Z_QP = SubsetbyValues(Z, 1, Z.nrows-1, x_QP);
    // vec y_QP = Z_QP.col(0);
    // mat X_QP = Z_QP.cols(1,(Z.nrows-1));
    ...
    for(int i=0; i<niter; i++){
        // Rcpp::Rcout << "Interation: " << i << " begins..." << std::endl;
        // Rcpp::Rcout << "Updating Q..." << std::endl;
        List DQP_post = UpdateQ(DQP_Q_post, DQP_Z_post, beta_post, gamma_post, 
                                quantile_levels, all_quantiles, alpha_scale, y, X, X_prime, group_ind, X_QP, //group_ind_QP, 
        ...
        beta_post = UpdateBeta(DQP_Q_post, beta_post, gamma_post, 
                               quantile_levels, all_quantiles, y, X, X_prime, group_ind, X_QP, //group_ind_QP, 
        ...
        gamma_post = UpdateGamma(DQP_Q_post, beta_post, gamma_post, 
                                 quantile_levels, all_quantiles, y, X, X_prime, group_ind, X_QP, //group_ind_QP, 
