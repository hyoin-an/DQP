...
// [[Rcpp::export]]
vec UpdateGamma(mat DQP, vec mu_x, vec mu_x_long, vec gamma_c, List quantile_levels, vec all_quantiles, 
                ...
                vec mu_1, mat Sigma_1_inv, double log_Sigma_1_det, int n_chunk=5)
{
    ...
    for (uword i = 0; i < ind_chunks.n_elem-1; i++){
        ...
        // Log Likelihood
        vec sigma_x_p = exp(X_QP * gamma_p);
        vec sigma_x_c = exp(X_QP * gamma_c);
