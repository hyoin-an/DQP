...
// [[Rcpp::export]]
vec interpolater(const vec& x, const vec& fx, const vec& x_new){ 
...
// [[Rcpp::export]]
mat interpolater_mat(const vec& x, const mat& fx_mat, const vec& x_new){
    // ** 1-dimensional interpolator ** 
    mat interpolated_fx(fx_mat.n_rows, x_new.size());

    for(int r = 0; r < fx_mat.n_rows; r++){
        vec interpolated = interpolater(x, fx_mat.row(r).t(), x_new);
...
vec zero_plus(vec v){
    ...
    for(int i=0; i<v.n_elem; i++){
        out(i) = std::max(v(i), 0.0);
    }
    return out;
}

// [[Rcpp::export]]
mat SplineDesignMatrix(mat X, vec knots, int d=0){
    ...
    if(d >= 1){ // d == 0; no spline
        ...
        for(int l=0; l<knots.n_elem; l++){
            X_sp = join_horiz(X_sp, pow(zero_plus(X.col(1)-knots(l)*ones(X_sp.n_rows)),d));
        }
    }
    return X_sp;
}