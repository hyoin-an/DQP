               ...
               vec mu_0, mat Sigma_0_inv, double log_Sigma_0_det, int n_chunk=2)
{
    ...
    vec mu_x_c = X_sp_QP * beta_c;
    vec mu_x_c_long = X_sp * beta_c;   // **NEW**
    ...
    for (uword i = 0; i < ind_chunks.n_elem-1; i++){
        ...
        // Rcpp::Rcout << "beta_c" << std::endl;
        // Rcpp::Rcout << beta_c << std::endl;
        // Rcpp::Rcout << "beta_p" << std::endl;
        // Rcpp::Rcout << beta_p << std::endl;
        // Rcpp::Rcout << "------------------" << std::endl;
        ...
        // Rcpp::Rcout << "log_beta_prior_c" << std::endl;
        // Rcpp::Rcout << log_beta_prior_c << std::endl;
        // Rcpp::Rcout << "log_beta_prior_p" << std::endl;
        // Rcpp::Rcout << log_beta_prior_p << std::endl;
        // Rcpp::Rcout << "------------------" << std::endl;

        // Log Likelihood
        vec mu_x_p = X_sp_QP * beta_p;
        vec mu_x_p_long = X_sp * beta_p;   // **NEW**
        ...
        // Rcpp::Rcout << "DQP_u_c" << std::endl;
        // Rcpp::Rcout << DQP_u_c << std::endl;
        // Rcpp::Rcout << "DQP_u_p" << std::endl;
        // Rcpp::Rcout << DQP_u_p << std::endl;
        // Rcpp::Rcout << "------------------" << std::endl;
        
        // **NEW: interpolationg Q in uniform scale **
        mat DQP_u_p_long = interpolater_mat(X_QP.col(1), DQP_u_p, X_prime.col(1)); // linear interpolating based on the lowest order of polynomials
        mat DQP_u_c_long = interpolater_mat(X_QP.col(1), DQP_u_c, X_prime.col(1)); // linear interpolating based on the lowest order of polynomials
        ...
        // Rcpp::Rcout << "log_like_c" << std::endl;
        // Rcpp::Rcout << log_like_c << std::endl;
        // Rcpp::Rcout << "log_like_p" << std::endl;
        // Rcpp::Rcout << log_like_p << std::endl;
        // Rcpp::Rcout << "------------------" << std::endl;
        ...
        // Rcpp::Rcout << "log_Q_density_given_beta_p" << std::endl;
        // Rcpp::Rcout << log_Q_density_given_beta_p << std::endl;
        // Rcpp::Rcout << "log_Q_density_given_beta_c" << std::endl;
        // Rcpp::Rcout << log_Q_density_given_beta_c << std::endl;
        // Rcpp::Rcout << "===================================" << std::endl;
        ...
        if (!Rcpp::NumericVector::is_na(log_a_star)) {
            ...
            if (log(u) <= log_a_star) {
                beta_c = beta_p;
                mu_x_c = X_sp_QP * beta_c;      // need to be updated here (fixed 8/30) 
                mu_x_c_long = X_sp * beta_c;    // need to be updated here (fixed 8/30)
                }
            }
    }
    return beta_c;
