...
double LogLikeC(mat DQP, vec all_quantiles, vec y, vec mu_x, vec sigma_x, vec group_ind) 
{
  ...
  for(int i = 0; i < n; ++i) {
    ...
    // DQP_col calculation //
      ...
      // 6/29/2023: note that meaning of long parameter is slightly different between Q and mu/sigma
      //            for mu/sigma, mu_long/sigma_long indicates mu/sigma value for X (with replicates), because they can be directly calculated and same value for same x location is guaranteed
      //            for Q, Q_long still indicates interpolated Q value, which is still for X_prime (without replicates), because we want same Q for same x location
      // t calculation //
