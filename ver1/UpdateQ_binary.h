...
// [[Rcpp::export]]
List UpdateQ(mat DQP_c_y, mat DQP_Z_c, vec y, vec mu_x, vec sigma_x, vec group_ind, List quantile_levels, vec all_quantiles,
             vec mu, mat Sigma, mat Sigma_inv, double log_Sigma_det, double delta, double alpha_scale=1, int n_chunk=5)
{
    ...
    for (uword i = 0; i < quantile_levels.size(); i++) {
        ...
        for (uword j = 0; j < quantile_targets.n_elem; j++) {
            ...
            // rowvec Q_Lu = DQP_c.row(arma::as_scalar(find(all_quantiles == tau_l)));
            // rowvec Q_Ru = DQP_c.row(arma::as_scalar(find(all_quantiles == tau_r)));
            ...
            for (uword i = 0; i < ind_chunks.n_elem-1; i++){
                ...
                double loglike_c = LogLikeC(DQP_c, all_quantiles, y, mu_x, sigma_x, group_ind);
                double loglike_p = LogLikeC(DQP_p, all_quantiles, y, mu_x, sigma_x, group_ind);
