             ...
             mat Sigma_p, double alpha_scale_p, int n_chunk=5)
{
    ...
    for (uword i = 0; i < ind_chunks.n_elem-1; i++){
        ...
        mat DQP_c_long = interpolator_mat(X_QP.col(1), DQP_c, X_prime.cols(locs)); // interpolating based on the lowest order of polynomials
        mat DQP_p_long = interpolator_mat(X_QP.col(1), DQP_p, X_prime.cols(locs)); // interpolating based on the lowest order of polynomials
        ...
        // Rcpp::Rcout << "Q_pu" << Q_pu << std::endl;
        // Rcpp::Rcout << "Q_cu" << Q_cu << std::endl;
        // Rcpp::Rcout << "loglike_c" << loglike_c << std::endl;
        // Rcpp::Rcout << "loglike_p" << loglike_p << std::endl;
        // Rcpp::Rcout << "log_a_star" << log_a_star << std::endl;
