                ...
                vec mu_1, mat Sigma_1_inv, double log_Sigma_1_det, int n_chunk=5)
{
    ...
    for (uword i = 0; i < ind_chunks.n_elem-1; i++){
        ...
        vec eta_p = eta_c;
        eta_p(ind) = eta_p_ind;
        vec eta_p_long = bilinearInterpolator(X_QP.cols(1,2), eta_p, X_prime.cols(locs)); // **NEW**
        vec eta_c_long = bilinearInterpolator(X_QP.cols(1,2), eta_c, X_prime.cols(locs)); // **NEW**
