...
// [[Rcpp::export]]
List UpdateQ(mat DQP_c_y, mat DQP_Z_c, vec beta, vec gamma, List quantile_levels, vec all_quantiles, double alpha_scale, 
             vec y, mat X, mat X_prime, vec group_ind, mat X_QP, uvec locs, uvec alocs,// alocs = {0, locs} intercept + two indexes of QP columns in X
             ...
             mat Sigma_p, double alpha_scale_p, int n_chunk=5)
{
    ...
    vec sigma_x = exp(X_QP * gamma.elem(alocs));  // on QP locations
    ...
    for (uword i = 0; i < ind_chunks.n_elem-1; i++){
        ...
        mat DQP_c_long = bilinearInterpolator_mat(X_QP.cols(1,2), DQP_c, X_prime.cols(locs)); // interpolating in uniform scale based on 2 columns (updated as of 10/22/2023)
        mat DQP_p_long = bilinearInterpolator_mat(X_QP.cols(1,2), DQP_p, X_prime.cols(locs)); // interpolating in uniform scale based on 2 columns (updated as of 10/22/2023)
