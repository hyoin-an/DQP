               ...
               vec mu_0, mat Sigma_0_inv, double log_Sigma_0_det, int n_chunk=2)
{
    ...
    for (uword i = 0; i < ind_chunks.n_elem-1; i++){
        ...
        // Log Likelihood
        vec mu_x_p = X_QP * beta_p.elem(alocs);
        vec mu_x_c = X_QP * beta_c.elem(alocs);
        vec mu_x_p_long = X * beta_p;   // **NEW**
        vec mu_x_c_long = X * beta_c;   // **NEW**
        vec sigma_x = exp(X_QP * gamma.elem(alocs));     // **NEW**
        ...
        mat DQP_u_p_long = bilinearInterpolator_mat(X_QP.cols(1,2), DQP_u_p, X_prime.cols(locs)); // interpolating based on the lowest order of polynomials
        mat DQP_u_c_long = bilinearInterpolator_mat(X_QP.cols(1,2), DQP_u_c, X_prime.cols(locs)); // interpolating based on the lowest order of polynomials
